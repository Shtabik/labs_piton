...
for year in years:
    for _ in range(200):  # 200 студентов на год
        ...
        total_score = round(sum(scores_ce.values())  + attest * 10 , 2)
...
plt.xticks(avg_scores.index.astype(int))
plt.grid()
plt.legend()
plt.show()
...
passing_score = df.groupby("Год")["Общий балл"].mean()
...
bars = plt.bar(spec_count.index, spec_count.values)
